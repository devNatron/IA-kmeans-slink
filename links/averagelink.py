...
def atualizaClusters(cluster_list):
    ...
    # Calcula a média e salva as novas distâncias no novo cluster
    for i in range(len(cluster_list[pos["x"]]["Distancias"])):
        if(cluster_list[pos["x"]]["Distancias"][i] != 0 and cluster_list[pos["y"]]["Distancias"][i] != 0):
            dist = (cluster_list[pos["x"]]["Distancias"][i] + cluster_list[pos["y"]]["Distancias"][i]) / 2
            cluster["Distancias"].append(dist)
...
def imprimirClusters(cluster_list, nClusters, nomeArquivo):
    arquivo=open("./" + nomeArquivo + "SaidaAverageLink_" + \
                 str(nClusters) + "C.clu", "w")
    arquivo.write("Particao: \n")
    for i in range(0, len(cluster_list)):
        arquivo.write("Cluster " + str(i+1) + ": " + str(
